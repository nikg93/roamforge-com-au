...
export const Route = createFileRoute("/product/$handle")({
  ...
  head: ({ params, loaderData }) => {
    ...
    // notFound() throws pre-render / on missing product; loader `errorComponent`
    // and `notFoundComponent` render — mark those responses noindex.
    if (!loaderData) {
      return {
        meta: [
    ...
    // Prefer Shopify's SEO fields where set, then fall back to product title /
    // stripped description. Never fabricate copy.
    ...
    // Only include SKU when Shopify has a real one — never leak the GID.
...
function ProductPage() {
  ...
  // Default to the first available variant (if any) so users don't have to
  // click before Add to Cart is enabled on multi-variant products.
