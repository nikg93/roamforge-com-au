...
const { useCartStore, classifyCartErrors, __resetCartQueueForTests } =
  await import("../../src/stores/cartStore.ts");
...
function product(id = "gid://shopify/Product/1", title = "AOB 12V 200psi Portable Air Compressor") {
  return {
    node: {
      ...
      description: "",
      handle: "aob",
      priceRange: { minVariantPrice: { amount: "100.00", currencyCode: "AUD" } },
      images: { edges: [] },
      variants: { edges: [] },
      options: [],
    },
...
export default {
  ...
  async "remove one item updates count and subtotal"() {
    ...
    stubShopify((name) =>
      name === "cartLinesRemove" ? { cartLinesRemove: { cart: { id: CART }, userErrors: [] } } : {},
    );
      ...
      name === "cartLines"
        ? {
            cart: {
              id: CART,
              totalQuantity: 1,
              lines: { edges: [{ node: { id: LINE, quantity: 1, merchandise: { id: VARIANT } } }] },
            },
          }
              ...
              userErrors:
                vars.lineIds[0] === LINE
                  ? [
                      {
                        field: ["lineIds", "0"],
                        message: "The merchandise line with id abc does not exist.",
                      },
                    ]
                  : [],
            },
  ...
  async "cart-not-found rejection clears the cart"() {
    reset({ cartId: CART, items: [item()] });
    stubShopify(() => ({
      cartLinesRemove: { cart: null, userErrors: [{ message: "Cart not found" }] },
    }));
    await useCartStore.getState().removeItem(VARIANT);
  ...
  async "failed mutation keeps the item and leaves the cart usable"() {
    reset({ cartId: CART, items: [item()] });
    stubShopify(() => ({
      cartLinesRemove: { cart: { id: CART }, userErrors: [{ message: "Something went wrong" }] },
    }));
    await useCartStore.getState().removeItem(VARIANT);
    const s = useCartStore.getState();
  ...
  async "syncCart heals stale line ids and drops server-absent rows"() {
    reset({
      ...
      items: [
        item({ lineId: "stale" }),
        item({ variantId: "v2", lineId: "l2", product: product("p2", "Gone") }),
      ],
    });
    stubShopify(() => ({
      cart: {
        id: CART,
        totalQuantity: 2,
        lines: { edges: [{ node: { id: LINE, quantity: 2, merchandise: { id: VARIANT } } }] },
      },
    }));
  ...
  async "syncCart preserves the cart on a transient network failure"() {
    ...
    assert.equal(
      ...
      1,
      "never destroy the cart on a flaky network",
    );
  },
